...
function publications(publications) {
    ...
    for (let i = 0; i < publications.length; i++) {
        ...
        let author = publications[i]['author'];
        let details = publications[i]['details'];
        let doi = publications[i]['doi'];
        fs.appendFileSync('latex.tex', `\\setlength{\\itemsep}{1pt}\n`);
        fs.appendFileSync('latex.tex', `\\item ${title}\n`);
