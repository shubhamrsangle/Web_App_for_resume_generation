...
class ExtraCurricularActivities extends React.Component {
  type = 7;
  state = {
      activity: "",
      modal: false,
  ...
  render() {
    return(
      <Card>
        <CardHeader>
          <button className="btn btn-link" id="extracurricularactivities">Extra Curricular Activities</button>
        </CardHeader>
        <UncontrolledCollapse toggler="#extracurricularactivities"><CardBody>
          <div className="row">
                {
                  this.props.fields.length === 0 ? <p>Nothing is Added</p> :
                  this.props.fields.map((nan,index,summa)=>{
                    return(
                      <div className="col-12 col-md-6 col-lg-4" key={"extracurricularactivity"+index}>
                      <div className="card col-auto">
                        <div className="card-header">
                          <h3 className="card-title">Extra Curricular Activity No:{index + 1}</h3>
                        </div>
                        <div className="card-body text-center">
                          <ul className="list-group list-group-flush">
                            <li className="list-group-item">
                              <strong>Activity</strong> : {nan.activity}
                            </li>
                          </ul>
                          ...
                          <Modal isOpen={this.state.editModal} toggle={()=>this.editToggle({},-1)} className={this.props.className}>
                            <ModalHeader toggle={this.editToggle}>Edit the Extra Curricular Activity</ModalHeader>
                            <ModalBody>
                            <form onSubmit={(e) => {this.editFunc(e);}} id="editEducation" name="editEducation">
                             <input type="text" className="form-control" name={"activity"} placeholder="Activity" value={this.state.activity}
           ...
           <Modal isOpen={this.state.modal} toggle={this.toggle} className={this.props.className}>
             <ModalHeader toggle={this.toggle}>Add an Extra Curricular Activity</ModalHeader>
             <ModalBody>
             <form onSubmit={this.submit} id="extracurricularactivity" name="extracurricularactivity">
              <input type="text" className="form-control" name={"activity"} placeholder="Activity"
               onChange={this.eventHandler}/>
             </form>
             </ModalBody>
             <ModalFooter>
                <button type="submit" form="extracurricularactivity">Submit</button>
